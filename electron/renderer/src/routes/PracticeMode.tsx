...
const PracticeMode: React.FC = () => {
  ...
  const [showTheoryResetModal, setShowTheoryResetModal] = useState(false);
  const [isTheoryResetting, setIsTheoryResetting] = useState(false);
  ...
  const handleResetTheoryProgress = async () => {
    if (selectedTags.size === 0 || isTheoryResetting) return;
    setIsTheoryResetting(true);
    try {
      await window.api.resetTheoryProgress(Array.from(selectedTags));
      setShowTheoryResetModal(false);
    } catch (error) {
      console.error('Failed to reset theory progress:', error);
    } finally {
      setIsTheoryResetting(false);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 min-h-[500px]">
          ...
          <GlassCard className="p-3 flex flex-col min-h-[500px]">
            ...
            <div className="mt-3 flex items-center justify-end gap-3 flex-wrap">
              <button
                onClick={() => setShowTheoryResetModal(true)}
                disabled={selectedTags.size === 0}
                className={`px-4 py-1.5 text-sm rounded-xl border border-red-400/50 bg-red-500/10 text-red-200 transition cursor-pointer ${
                  selectedTags.size === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-500/20 hover:border-red-400/70'
                }`}
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white mb-1">Reset Theory Progress</h3>
                <p className="text-sm text-zinc-400">
                  This will reset your answered counts for the selected lessons. You can retake them as if they were brand new.
                </p>
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 mt-6">
              <button
                onClick={() => setShowTheoryResetModal(false)}
                disabled={isTheoryResetting}
                className="px-4 py-2 text-sm rounded-xl bg-zinc-800 hover:bg-zinc-700 transition cursor-pointer disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                onClick={handleResetTheoryProgress}
                disabled={isTheoryResetting}
                className="px-4 py-2 text-sm rounded-xl bg-red-600 hover:bg-red-500 text-white font-medium transition cursor-pointer disabled:opacity-50 flex items-center gap-2"
              >
                {isTheoryResetting ? (
